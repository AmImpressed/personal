...
function split_and_matrix(matrix) { // <- function to construct matrix
    let rows = iris.split('\n');
    for (let row of rows) {
...
// Function to traverse the matrix and sum numeric values per iris type
function read_matrix(matrix) { // <- function to traverse matrix per class
    // Loop starting at 1 to skip header row
    for (let i = 1; i < matrix.length; i++) {
        let element = matrix[i];
        // Example element: [ '5.1', '3.5', '1.4', '0.2', 'Iris-setosa' ] 
        if (element[4] === "Iris-setosa") {
            ...
            console.log(element[4] + " is not an accepted input.");
        }
    }
}

// Call function: compute averages
function compute_averages() { // <- function to compute averages for iris type
    // Iris-setosa averages
...
// Moved Function Calls and Matrix initialzation 

// 1) Create an empty matrix and populate it from the iris data
let matrix = [];
split_and_matrix(matrix);

// 2) Read the matrix and sum/count numeric values per iris type
read_matrix(matrix);

// 3) Compute averages using the sums and counts
compute_averages();
